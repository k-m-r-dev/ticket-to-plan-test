...
| Commit cadence | milestone — single commit after M005 verified; skill overrides profile `slice` cadence |
...
- **Commit Message Format** (`.gsd/workflow/milestone-workflow.md`) — `feat(local-todo-http-api): summary`; never use local IDs `M005`/`S0x` in commits or PR titles
