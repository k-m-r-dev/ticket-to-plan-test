...
Implement all `/todos` endpoints, `completed` query filter, title validation (1..=200 chars), PATCH body rules, and consistent error JSON envelope.

## Tasks

### T01 — Domain model + repository layer

- **Inputs:** Assumed `Todo` fields — UUID `id`, `title`, `completed`, `created_at`, `updated_at`
...
- **DoD:** Server sets `id`, `created_at`, `updated_at` on create; `updated_at` refreshed on successful PATCH
...
- **Inputs:** Assumed shape `{ "error": { "code", "message" } }`
