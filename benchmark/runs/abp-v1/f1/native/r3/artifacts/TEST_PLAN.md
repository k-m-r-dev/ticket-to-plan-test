# Test plan — Todo HTTP API (native arm, ABP f1, r3)

Integration tests use in-process HTTP (Axum `TestClient` / tower service tests) against an isolated SQLite database per test or per suite. All tests live in `tests/integration.rs` with a shared `setup_app()` helper.
...
| Module | Cases |
| --- | --- |
| `service.rs` | Title trim, empty rejection, 200-char boundary, 201-char rejection |
| `service.rs` | `parse_completed_filter` accepts `true`/`false`, rejects other values |
| `error.rs` | Each `AppError` variant maps to correct status + code |
