...
- Implement the Axum-based HTTP server in the existing `apps/todo-api` skeleton, exposing `POST /todos`, `GET /todos` (with `completed` filter), `GET /todos/:id`, `PATCH /todos/:id`, `DELETE /todos/:id`, and `GET /health`.
...
- **Code location**: `apps/todo-api/` (existing skeleton crate expanded into full implementation).
- **New dependencies**: `axum`, `tokio`, `sqlx` (sqlite feature), `serde`/`serde_json`, `uuid`, `chrono` for RFC 3339 timestamps, `tower`/`axum::test` helpers for integration tests.
